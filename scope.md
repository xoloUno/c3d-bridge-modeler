...
This chain is computed per girder per support, accounting for cross slope. If `cap_slope_follows_deck` is true, the cap top slopes and pedestal heights vary to provide level bearing seats.

The elevation chain module is **pure math with no C3D dependency** -- it takes profile elevation, cross slope, and component dimensions as inputs and returns all computed elevations. Testable on macOS and usable as a standalone calculation check.
...
1. **`FOLLOW_ALIGNMENT` (recommended default for curved bridges):** Each girder centerline and each edge-of-deck line is a constant lateral offset from the alignment. Same approach as Civil 3D corridors and OpenBridge Modeler -- the alignment already encodes the curve geometry (arcs, spirals), so each girder path is just `alignment.PointAtStationOffset(station, offset)` sampled at regular intervals. Girders are concentric with the alignment and with each other. The cross-section is swept along this path.

2. **`CURVED_RADIUS`:** Each girder has an independently specified constant radius (not necessarily concentric with the alignment or each other). This handles bridges that widen along a curve, where girder spacing increases and each girder gets a unique radius. The centerline is a circular arc between support points at the specified radius.
...
**Result:** `CreateSweptSolid` produces curved NURBS solids (not faceted/chorded). The surfaces are smooth B-spline faces that render correctly in Hidden visual style and measure correctly with DIMRADIUS on the sub-alignment (which IS a true arc).
...
The surface supports standard C3D workflows: spot elevation labels, slope arrows, elevation difference analysis vs. FG, and sharing with the roadway team for composite surface assembly.

### IFC Property Set Attachment

Each solid gets IFC classification at creation time via AutoCAD Property Sets:

1. Property Set Definitions are pre-built in the template drawing (or created by the tool if missing)
2. After solid creation, attach the Property Set: `PropertyDataServices.AddPropertySet(solidId, propSetDefId)`
3. Set property values (IFC entity type, predefined type, bridge name, element ID)

The template drawing ships with standard IFC PropSet definitions. No manual object mapping needed on export.
