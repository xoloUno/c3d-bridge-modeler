...
- Single-arc edge polylines (`src/bridge_lines.py`) — each edge polyline is a 2-vertex polyline with one arc segment from start support to end support. The arc bulge is computed from a midpoint sample on the alignment curve at perpendicular offset (sagitta / half-chord = `tan(θ/4)`), so `DIMRADIUS` reports the correct edge radius. On straight alignments the midpoint is collinear → bulge = 0 → straight segment. Schema version `v7-single-arc`; stale polylines self-heal on first post-update run.
